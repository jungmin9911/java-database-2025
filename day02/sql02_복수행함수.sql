...
SELECT count(employee_id)  -- 위와 같음
  FROM employees;

-- SUM(컬럼값) 합계, 컬럼은 숫자형만 가능
-- employees 206 salary 8300 삭제
SELECT sum(salary)
  FROM employees;

-- AVG(숫자형컬럼) 평균
-- 컬럼에 NULL값이 있으면 이를 제외하고 계산하기 때문에 잘못된 값 도출
-- 금액이나 수량을 계산하는 컬럼의 NULL값은 전처리해줘야함
SELECT avg(salary)
  FROM employees;

SELECT count(salary)
  FROM employees;    -- NULL은 count 안 됨

-- MIN(숫자형컬럼 | 문자형컬럼), MAX()
SELECT max(first_name), min(salary), max(salary)
  FROM employees;

/* GROUP BY 연계, 그룹화
 * GROUP BY를 사용하면 SELECT절에는 GROUP BY에 사용한 컬럼과 집계함수, 일반함수만 사용할 수 있음 */
...
-- employees에서 부서와 직군별 급여총액과 직원수를 출력
-- department_id가 30에서 90사이이고, 부서직구별급여총액이 2만달러 이상인 데이터만 출력
SELECT department_id, job_id, sum(salary) AS 부서직군별급여총액
	 , count(*)
  FROM employees
...
-- ORDER BY에는 컬럼의 순서(1번부터 시작)으로 컬럼명을 대체가능
SELECT department_id, job_id, sum(salary) AS 부서직군별급여총액
	 , count(*)
  FROM employees
 WHERE department_id BETWEEN 30 AND 90
...
-- 옆으로 각달별로 스프레드
SELECT CASE to_char(hire_date, 'MM') WHEN '01' THEN count(*), 0 AS "1월"
...
SELECT decode(to_char(hire_date, 'MM'), '01', count(*), 0) AS "1월"
	 ...
	 , decode(to_char(hire_date, 'MM'), '11', count(*), 0) AS "12월"
	 , decode(to_char(hire_date, 'MM'), '12', count(*), 0) AS "12월"
  FROM employees
GROUP BY to_char(hire_date, 'MM')
ORDER BY to_char(hire_date, 'MM');

-- RANK() 등수 공동등수 번호 띄우기, DENSE_RANL() 등수번호가 순차적으로 올라감
-- ROW_NUMBER() 현재 데이터 행번호 출력
SELECT employee_id, last_name, salary
	 , rank() OVER (ORDER BY salary desc) AS "랭크"  -- 잘 안 씀
