...
- DML : [INSERT쿼리](./day04/sql01_INSERT.sql), [UPDATE/DELETE](./day04/sql02_UPDATE_DELETE.sql)
...
- 트랜잭션 : [트랜잭션쿼리](./day04/sql03_트랜잭션.sql)
    ...
    - 은행에서 돈을 찾을때 아주 많은 테이블 접근해서 일을 처리
        ...
        - ROLLBACK 트랜잭션 종료X / COMMIT 트랜잭션 종료O
        ```sql
...
- 제약조건(CONSTRAINT) : [제약조건](./day04/sql04_제약조건.sql)
    - 잘못된 데이터가 들어가지 않도록 막는 기법
        ...
        - DEFAULT : NULL위치 입력시 기본값이 입력되도록 하는 제약조건
        ```sql
        CREATE TABLE 테이블명(
            컬럼 생성시 제약조건 추가
        );
        ...
        ```
- INDEX : [INDEX](./day04/sql05_인덱스.sql)
    - 책의 찾아보기와 동일한 기능
...
- VIEW : [뷰쿼리](./day05/sql01_뷰.sql)
    ...
    - 기존 테이블 중 개인정보나 중요한 부분이 있으면 제외 후 보일 수 있음
    - INSERT, UPDATE, DELETE 가능(단일뷰에서만)
        ```sql
        CREATE VIEW 뷰명
        AS
            SELECT 쿼리;
...
- 시퀀스 : [시퀀스](./day05/sql03_시퀀스.sql)
    ...
    - Oracle만 존재. 타DB보다 자동증가값 사용 불편
        ...
        [CYCLE]         -- 최대증가값에 도달하면 다시 처음 1로 돌아갈 것인지
...
- 사용자 계정 권한 : [쿼리](./day05/sql04_사용자계정관리.sql)
    - 사용자를 생성 후 권한(Role)을 부여해야 스키마를 사용할 수 있음
    ```sql
    -- 권한 부여
